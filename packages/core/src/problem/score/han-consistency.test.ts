 ...
 * 翻数と役の内訳の整合性
 *
 * アプリは翻数を `calculateScoreForTehai` から、内訳を `detectYaku` から
 * 取り、後付けの翻（役牌の照合・立直・裏ドラ）を両方に足す。
 * `generateScoreQuestion` が内訳の合計を翻数の正典にすることで揃えている。
 * ライブラリ 0.5 までは両 API が同じ手牌で食い違うことがあり（門前の清一色・
 * 混一色・混全帯么九を含む手で 30000 手あたり 19 件）、その頃からの検査。
 *
