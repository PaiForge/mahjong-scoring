...
export interface QuestionGeneratorOptions {
   ...
   * 出題符の絞り込み
   *
   * 出題の符の帯を切る出題（30〜50符の昇級試験など）と、符が固定される役
   * だけを出す出題（七対子・平和の昇級試験など）が渡す。後者は防波堤として
   * 持つ。役の判定（`detectYaku`）と点数計算（`calculateScoreForTehai`）は
   * 同じ解釈（高点法）を採るため、平和が立った手の符が 20/30符 以外になる
   * ことは無い。ライブラリ 0.5 までは両者が解釈を独立に選び、
   * 999m 111p 222p 333p + 単騎 のような手で「役には平和が立つのに点数は
   * 暗刻側の 50符」が出ていた（平和を名指しした出題の約 0.05%）。
   * `allowedFu` は最終的な点数計算の符で弾くので、仮に再発しても落とせる。
   */
