...
import { saveMachiFuResult } from '../save-result';

describe('saveMachiFuResult', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  describe('delegation to savePracticeResult', () => {
    it('calls savePracticeResult with menuType "machi_fu" and leaderboardKey "default"', async () => {
      mockSavePracticeResult.mockResolvedValue({ success: true });

      await saveMachiFuResult({
        correctAnswers: 10,
        incorrectAnswers: 2,
        timeTaken: 45,
      });

      expect(mockSavePracticeResult).toHaveBeenCalledWith('machi_fu', 'default', {
        score: 10,
        incorrectAnswers: 2,
        timeTaken: 45,
      });
