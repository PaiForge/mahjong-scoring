 ...
 *
 * ただし未ログインのときはアンカーへ行かず、ページ先頭へ着地させる。設定項目は
 * 会員限定で、未ログインではぼかしと会員登録 CTA に覆われて操作できないため、
 * 目当ての項目まで送り届けても読者にできることが無い。しかも CTA は覆っている
 * 領域全体の中央にあり、項目までスクロールすると画面外へ置き去りになる。
 * 「スクロールしない」だけでは足りない — ハッシュ付き遷移では Next が
 * ページ先頭へのスクロールを行わない（ハッシュへのスクロールに置き換わり、
 * それも上記の通り不発に終わる）ため、前ページのスクロール位置が残って
 * ページ中腹に着地してしまう。先頭へ明示的に戻して素の遷移と同じ着地にする。
 *
