 ...
 *
 * `allowedFu` が要る理由: 役の判定と点数計算は手牌の解釈を独立に選ぶため、
 * 面子の取り方が複数ある手（999m 111p 222p 333p + 単騎 など）では役に平和が
 * 立つのに点数は暗刻側の 50符 で出る。`requiredYaku` だけでは落とせず、
 * 平和を名指しした出題の約 0.05% がこれに当たる。ミス1回で終わる試験では
 * 1回の受験あたり約 0.5% が「平和なのに 20/30符 でない」問題を踏む計算で、
 * 合否が記録に残る試験としては許容できない。
 *
