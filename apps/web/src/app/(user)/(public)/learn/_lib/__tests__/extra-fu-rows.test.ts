...
describe("buildExtraFuRows", () => {
  it("符が同じ並びを1行にまとめる", () => {
    expect(buildExtraFuRows("menzen")).toEqual([
  ...
  it("門前手は積み上げ0符（平和）を含めない", () => {
    expect(buildExtraFuRows("menzen").every((row) => row.from >= 2)).toBe(true);
  });

  it("副露はロンが門前より10符低く、ツモが上回ることがある", () => {
    expect(buildExtraFuRows("furo")).toEqual([
